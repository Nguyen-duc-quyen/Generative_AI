...
class Linear_AutoEncoder(nn.Module):
    def __init__(self, in_features, enc_hidden_features, embedding_features, dec_hidden_features, k):
        super().__init__()
        self.k = k
    ...
    def forward(self, x):
        ...
        # Only keep k largest activations, zero out the rest
        kth_value, _ = torch.kthvalue(-emb, k=self.k, dim=-1, keepdim=True)
        kth_value = -kth_value
        mask = (emb >= kth_value).float()
        emb = emb * mask
